...
public abstract class DocumentStoreIndexerBase implements Closeable{
    ...
    private List<FlatFileStore> buildFlatFileStore(NodeState checkpointedState, CompositeIndexer indexer, Predicate<String> pathPredicate, Set<String> preferredPathElements) throws IOException {
    ...
    public FlatFileStore buildFlatFileStore() throws IOException, CommitFailedException {
        ...
        List<FlatFileStore> storeList = buildFlatFileStore(checkpointedState, null, predicate, preferredPathElements);
        log.info("FlatFileStore built at {}. To use this flatFileStore in a reindex step, set System Property-{} with value {}",
                storeList.get(0).getFlatFileStorePath(), OAK_INDEXER_SORTED_FILE_PATH, storeList.get(0).getFlatFileStorePath());
    ...
    public void reindex() throws CommitFailedException, IOException {
        ...
        List<FlatFileStore> storeList  = buildFlatFileStore(checkpointedState, indexer, null, null);
...
//            log.info("=====index job start running for {}", entry.getPath());
