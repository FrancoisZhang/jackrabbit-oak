...
public class ElasticQueryExplainTest extends ElasticAbstractQueryTest {
    ...
    public void testQuerySuggest() throws Exception {
        ...
        assertEventually(() -> {
            assertResultList(Arrays.asList(SUGGEST_TITLE + "-1", SUGGEST_TITLE + "-2", SUGGEST_TITLE + "-3"),
                    executeSuggestQuery(suggestQuery, SQL2));
    ...
    @Test
    public void testQuerySpellcheck() throws Exception {
        configureIndex();
        prepareTestAssets();
        String spellcheckQuery = "SELECT [rep:spellcheck()] FROM [dam:Asset] as s WHERE SPELLCHECK('respons')";
        assertEventually(() -> {
            assertResultList(Arrays.asList("response"), executeSpellcheckQuery(spellcheckQuery, SQL2));
            LOG.info("=====explain spellcheck query: {}", explain(spellcheckQuery, SQL2));
        });
    }
    ...
    public void testQueryFulltext() throws Exception {
        configureIndex();
        prepareTestAssets();
        String query = "select [jcr:path] from [dam:Asset] where contains(*, 'response')";
        assertEventually(() -> {
//            assertResultList(Arrays.asList("/test/asset1", "/test/asset2", "/test/asset3"), executeQuery(query, SQL2));
            LOG.info("=====explain fulltext query: {}", explain(query, SQL2));
        });
    }
    ...
    public void testQueryChildren() throws Exception {
        configureIndex();
        prepareTestAssets();
        String query = "/jcr:root/test//element(*, dam:Asset)";
        assertEventually(() -> {
            assertResultList(Arrays.asList("/test/asset1", "/test/asset2", "/test/asset3"), executeQuery(query, XPATH));
            LOG.info("=====explain query: {}", explain(query, XPATH));
        });
    }
    ...
    private List<String> executeSpellcheckQuery(String query, String language) {
        List<String> results = new ArrayList<String>();
        Result result = null;
        try {
            result = executeQuery(query, language, NO_BINDINGS);
        ...
        for (ResultRow row : result.getRows()) {
            results.add(row.getValue("rep:spellcheck()").toString());
        }
        return results;
    }

    // utils
    private void assertResultList(@NotNull List<String> expected, @NotNull List<String> actual) {
    ...
    private void configureIndex() throws CommitFailedException {
        ...
        IndexDefinitionBuilder.PropertyRule titlePropertyRule = indexRule.property("title").analyzed().nodeScopeIndex().useInSuggest().useInSpellcheck();
